/**
 * Đề thi / slide kiểu: mỗi nút có giá trị đánh giá h(n).
 * Chỉ dùng cho tab Best-First & Leo đồi (không dùng chung với DFS/BFS/DLS).
 */
export const heuristicGraphsData = [
  {
    id: 0,
    level: 1,
    name: "Đề 1: A → B (h trên nút, slide chuẩn)",
    nodes: ["A", "B", "C", "D", "E", "F", "G", "H", "I", "K"],
    edges: {
      A: ["C", "D", "E"],
      C: ["F"],
      D: ["F", "I"],
      E: ["K", "G"],
      F: ["B"],
      I: ["B", "G"],
      G: ["B", "H"],
      H: ["B"],
      B: [],
      K: [],
    },
    start: "A",
    goal: "B",
    heuristic: {
      A: 20,
      B: 0,
      C: 15,
      D: 6,
      E: 7,
      F: 10,
      G: 5,
      H: 3,
      I: 8,
      K: 12,
    },
    positions: {
      A: [175, 28],
      C: [115, 105],
      D: [175, 105],
      E: [235, 105],
      F: [115, 185],
      I: [235, 185],
      K: [285, 185],
      H: [95, 255],
      G: [175, 255],
      B: [175, 322],
    },
  },
  {
    id: 1,
    level: 1,
    name: "Đề 2: A → K (đồ thị có h, kiểu đề giấy)",
    nodes: ["A", "B", "C", "D", "E", "F", "G", "H", "I", "K"],
    edges: {
      A: ["B", "C", "D"],
      B: ["E"],
      C: ["E", "F"],
      D: ["G", "H"],
      E: ["K"],
      F: ["K", "H"],
      G: ["H"],
      H: ["I"],
      I: ["K"],
      K: [],
    },
    start: "A",
    goal: "K",
    heuristic: {
      A: 20,
      B: 15,
      C: 6,
      D: 7,
      E: 10,
      F: 8,
      G: 13,
      H: 3,
      I: 5,
      K: 0,
    },
    positions: {
      A: [175, 28],
      B: [95, 100],
      C: [175, 100],
      D: [255, 100],
      E: [115, 175],
      F: [175, 175],
      G: [230, 175],
      H: [285, 175],
      I: [285, 248],
      K: [175, 318],
    },
  },
  {
    id: 2,
    level: 1,
    name: "Đề 3: S → T (h trên nút, nhánh rẽ nhiều)",
    nodes: ["S", "A", "B", "C", "D", "E", "F", "G", "T"],
    edges: {
      S: ["A", "B", "C"],
      A: ["D", "E"],
      B: ["F"],
      C: ["F", "G"],
      D: ["T"],
      E: ["T"],
      F: ["E", "T"],
      G: ["T"],
      T: [],
    },
    start: "S",
    goal: "T",
    heuristic: {
      S: 18,
      A: 12,
      B: 8,
      C: 10,
      D: 7,
      E: 4,
      F: 6,
      G: 5,
      T: 0,
    },
    positions: {
      S: [175, 30],
      A: [80, 105],
      B: [175, 105],
      C: [270, 105],
      D: [55, 190],
      E: [115, 190],
      F: [225, 190],
      G: [290, 190],
      T: [175, 315],
    },
  },
  {
    id: 3,
    level: 2,
    name: "Đề 4: P → Z (dạng cây ngắn)",
    nodes: ["P", "Q", "R", "S", "T", "U", "Z"],
    edges: {
      P: ["Q", "R", "S"],
      Q: ["T"],
      R: ["U"],
      S: ["Z"],
      T: [],
      U: [],
      Z: [],
    },
    start: "P",
    goal: "Z",
    heuristic: {
      P: 19,
      Q: 9,
      R: 11,
      S: 5,
      T: 7,
      U: 6,
      Z: 0,
    },
    positions: {
      P: [175, 28],
      Q: [95, 108],
      R: [175, 108],
      S: [255, 108],
      T: [95, 198],
      U: [175, 198],
      Z: [255, 198],
    },
  },
  {
    id: 4,
    level: 2,
    name: "Đề 5: M → H (có nhánh hội tụ)",
    nodes: ["M", "N", "O", "P", "Q", "R", "H"],
    edges: {
      M: ["N", "O"],
      N: ["P", "Q"],
      O: ["Q", "R"],
      P: ["H"],
      Q: ["H"],
      R: ["H"],
      H: [],
    },
    start: "M",
    goal: "H",
    heuristic: {
      M: 14,
      N: 9,
      O: 8,
      P: 6,
      Q: 5,
      R: 4,
      H: 0,
    },
    positions: {
      M: [175, 28],
      N: [105, 108],
      O: [245, 108],
      P: [70, 200],
      Q: [175, 200],
      R: [280, 200],
      H: [175, 300],
    },
  },
  {
    id: 5,
    level: 2,
    name: "Đề 6: A → J (đồ thị có chu trình nhẹ)",
    nodes: ["A", "B", "C", "D", "E", "F", "G", "J"],
    edges: {
      A: ["B", "C"],
      B: ["D", "E"],
      C: ["E", "F"],
      D: ["B", "G"],
      E: ["J"],
      F: ["J"],
      G: ["J"],
      J: [],
    },
    start: "A",
    goal: "J",
    heuristic: {
      A: 16,
      B: 12,
      C: 10,
      D: 11,
      E: 6,
      F: 7,
      G: 4,
      J: 0,
    },
    positions: {
      A: [175, 28],
      B: [105, 110],
      C: [245, 110],
      D: [70, 228],
      E: [148, 190],
      F: [245, 190],
      G: [315, 162],
      J: [175, 315],
    },
  },
  {
    id: 6,
    level: 2,
    name: "Đề 7: S → G (bẫy cực trị địa phương)",
    nodes: ["S", "A", "B", "C", "D", "E", "G"],
    edges: {
      S: ["A", "B", "C"],
      A: ["D"],
      B: ["E"],
      C: ["G"],
      D: [],
      E: ["G"],
      G: [],
    },
    start: "S",
    goal: "G",
    heuristic: {
      S: 13,
      A: 6,
      B: 5,
      C: 7,
      D: 2,
      E: 4,
      G: 0,
    },
    positions: {
      S: [175, 28],
      A: [90, 108],
      B: [175, 108],
      C: [260, 108],
      D: [90, 200],
      E: [175, 200],
      G: [175, 300],
    },
  },
  {
    id: 7,
    level: 3,
    name: "Đề 8: X → T (nhiều đường đi)",
    nodes: ["X", "A", "B", "C", "D", "E", "F", "T"],
    edges: {
      X: ["A", "B", "C"],
      A: ["D"],
      B: ["D", "E"],
      C: ["F"],
      D: ["T"],
      E: ["T"],
      F: ["T"],
      T: [],
    },
    start: "X",
    goal: "T",
    heuristic: {
      X: 17,
      A: 9,
      B: 7,
      C: 8,
      D: 4,
      E: 5,
      F: 3,
      T: 0,
    },
    positions: {
      X: [175, 30],
      A: [75, 110],
      B: [160, 110],
      C: [245, 110],
      D: [95, 205],
      E: [175, 205],
      F: [255, 205],
      T: [175, 315],
    },
  },
  {
    id: 8,
    level: 3,
    name: "Đề 9: L → K (nhánh sâu)",
    nodes: ["L", "M", "N", "O", "P", "Q", "K"],
    edges: {
      L: ["M", "N"],
      M: ["O"],
      N: ["P", "Q"],
      O: ["K"],
      P: ["K"],
      Q: ["K"],
      K: [],
    },
    start: "L",
    goal: "K",
    heuristic: {
      L: 15,
      M: 11,
      N: 9,
      O: 7,
      P: 5,
      Q: 6,
      K: 0,
    },
    positions: {
      L: [175, 28],
      M: [105, 108],
      N: [245, 108],
      O: [105, 198],
      P: [215, 198],
      Q: [285, 198],
      K: [175, 315],
    },
  },
  {
    id: 9,
    level: 3,
    name: "Đề 10: U → V (đồ thị tổng hợp)",
    nodes: ["U", "A", "B", "C", "D", "E", "F", "V"],
    edges: {
      U: ["A", "B"],
      A: ["C", "D"],
      B: ["D", "E"],
      C: ["F"],
      D: ["F", "V"],
      E: ["V"],
      F: ["V"],
      V: [],
    },
    start: "U",
    goal: "V",
    heuristic: {
      U: 20,
      A: 13,
      B: 12,
      C: 9,
      D: 8,
      E: 6,
      F: 4,
      V: 0,
    },
    positions: {
      U: [175, 28],
      A: [105, 108],
      B: [245, 108],
      C: [70, 200],
      D: [175, 200],
      E: [280, 200],
      F: [175, 285],
      V: [175, 345],
    },
  },
  {
    id: 10,
    level: 4,
    name: "Đề 11: S → G (Đồ thị mở rộng)",
    nodes: ["S", "A", "B", "C", "D", "E", "F", "H", "G"],
    edges: {
      S: ["A", "B", "C"],
      A: ["D", "E"],
      B: ["E", "F"],
      C: ["F", "H"],
      D: ["G"],
      E: ["G"],
      F: ["G"],
      H: ["G"],
      G: [],
    },
    start: "S",
    goal: "G",
    heuristic: {
      S: 15,
      A: 10,
      B: 12,
      C: 11,
      D: 8,
      E: 5,
      F: 6,
      H: 9,
      G: 0,
    },
    positions: {
      S: [175, 30],
      A: [60, 100],
      B: [175, 100],
      C: [290, 100],
      D: [40, 200],
      E: [130, 200],
      F: [220, 200],
      H: [310, 200],
      G: [175, 300],
    },
  },
  {
    id: 11,
    level: 4,
    name: "Đề 12: Mê cung đơn giản",
    nodes: ["Start", "N1", "N2", "N3", "N4", "End"],
    edges: {
      Start: ["N1", "N2"],
      N1: ["N3"],
      N2: ["N4"],
      N3: ["End"],
      N4: ["End"],
    },
    start: "Start",
    goal: "End",
    heuristic: {
      Start: 10,
      N1: 8,
      N2: 5,
      N3: 3,
      N4: 7,
      End: 0,
    },
    positions: {
      Start: [175, 30],
      N1: [100, 120],
      N2: [250, 120],
      N3: [100, 220],
      N4: [250, 220],
      End: [175, 320],
    },
  },
  {
    id: 12,
    level: 5,
    name: "Đề 13: Đồ thị Bẫy Heuristic",
    nodes: ["S", "A", "B", "C", "D", "G"],
    edges: {
      S: ["A", "B"],
      A: ["C"],
      B: ["D"],
      C: ["G"],
      D: ["G"],
    },
    start: "S",
    goal: "G",
    heuristic: {
      S: 10,
      A: 2,
      B: 8,
      C: 5,
      D: 1,
      G: 0,
    },
    positions: {
      S: [175, 30],
      A: [100, 120],
      B: [250, 120],
      C: [100, 220],
      D: [250, 220],
      G: [175, 320],
    },
  },
  {
    id: 13,
    level: 5,
    name: "Đề 14: Nhánh Cây Cân Bằng",
    nodes: ["R", "L", "RR", "LL", "LR", "RL", "G"],
    edges: {
      R: ["RR", "RL"],
      L: ["LL", "LR"],
      RR: ["G"],
      RL: ["G"],
      LL: ["G"],
      LR: ["G"],
    },
    start: "R",
    goal: "G",
    heuristic: { R: 10, L: 8, RR: 5, LL: 6, LR: 7, RL: 4, G: 0 },
    positions: {
      R: [200, 30],
      L: [100, 100],
      RR: [300, 100],
      LL: [50, 200],
      LR: [150, 200],
      RL: [250, 200],
      G: [200, 300],
    },
  },
  {
    id: 14,
    level: 6,
    name: "Đề 15: Đề Thi Chuẩn (Hình ảnh)",
    nodes: ["B", "A", "F", "G", "H", "E", "D", "C", "L"],
    edges: {
      B: ["F", "G", "H", "A"],
      F: ["E"],
      G: ["E", "H"],
      H: ["D", "C"],
      A: ["C"],
      E: ["D"],
      C: ["D", "L"],
      D: ["L"],
      L: [],
    },
    start: "B",
    goal: "L",
    heuristic: { B: 15, A: 5, F: 14, G: 6, H: 8, E: 10, D: 3, C: 4, L: 0 },
    positions: {
      B: [200, 30],
      A: [380, 80],
      F: [40, 120],
      G: [160, 120],
      H: [280, 120],
      E: [80, 240],
      D: [240, 240],
      C: [380, 240],
      L: [240, 340],
    },
  },
];
