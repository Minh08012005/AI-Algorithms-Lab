...
import { branchBoundGraphsData } from "../data/branchBoundGraphsData";
import { getBranchAndBoundTrace } from "./branchAndBoundEngine";

const markerId = "branch-bound-arrow";
const STORAGE_KEY = "ai-lab-branch-bound-practice-v1";
...
export default function BranchAndBoundModule() {
  ...
  const [selectedLevel, setSelectedLevel] = useState(
    () => branchBoundGraphsData[initialSession?.graphIdx ?? 0]?.level ?? 1,
  );
  ...
  const levelOptions = useMemo(() => {
    const levels = Array.from(new Set(branchBoundGraphsData.map((g) => g.level))).sort(
      (a, b) => a - b,
    );
    return levels;
  }, []);
  ...
  const visibleGraphEntries = useMemo(
    () =>
      branchBoundGraphsData
        .map((g, idx) => ({ g, idx }))
        .filter(({ g }) => g.level === selectedLevel),
    [selectedLevel],
  );

  const graph = branchBoundGraphsData[graphIdx];
  const { rows, finalPath, finalCost } = useMemo(() => getBranchAndBoundTrace(graph), [graph]);
  ...
  const handleLevelChange = (nextLevel) => {
    ...
    const firstMatchIdx = branchBoundGraphsData.findIndex((g) => g.level === nextLevel);
  ...
  useEffect(() => {
    ...
    const firstMatchIdx = branchBoundGraphsData.findIndex((g) => g.level === selectedLevel);
    if (firstMatchIdx < 0) return;
    handleGraphChange(firstMatchIdx);
  }, [selectedLevel, graph]);
  ...
  const handleHint = () => {
    ...
    setInputs((prev) => ({
      ...prev,
      l1: currentRow.listL1 || "",
      l: currentRow.listL || "",
    }));
    setFeedback({ type: "success", text: `Gợi ý đã điền DS L1 và L. -${HINT_PENALTY}%` });
  };
  ...
  const handleCheck = () => {
    ...
    if (currentRow.isGoal) {
      if (normalize(inputs.goalText) !== normalize(currentRow.goalText)) {
        ...
        setFeedback({
          type: "error",
          text: "Dòng TTKT tạm thời chưa đúng.",
        });
        return;
      }
      ...
      if (nextStep >= rows.length) {
        ...
        setFeedback({ type: "success", text: "Đúng TTKT tạm thời. Tiếp tục mở rộng." });
      }
      return;
    }
    ...
    if (!Array.isArray(inputs.rows)) {
      setFeedback({
        type: "error",
        text: "Dữ liệu dòng không hợp lệ.",
      });
      return;
    }
    ...
    if (currentRow.details.length === 0) {
      const newIndex = history.length;
      setHistory([...history, currentRow]);
      setLastCorrectIndex(newIndex);
      const nextStep = step + 1;
      ...
      if (nextStep >= rows.length) {
        ...
        setFeedback({ type: "success", text: "Đúng trạng thái không có con hợp lệ." });
      }
      return;
    }
    ...
    const allOk =
      ...
      normalize(actual.l1) === normalize(expected.l1) &&
    ...
    if (!allOk) {
      ...
      setFeedback({
        type: "error",
        text: "Sai một hoặc nhiều cột (TTK/k/h/g/f/DS L1/L). Kiểm tra lại.",
      });
      return;
    }
    ...
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-6xl mx-auto">
      <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 mb-6 flex flex-wrap justify-between items-center gap-4">
        <div className="flex items-center gap-4">
          <div className="p-3 rounded-xl text-white shadow-lg bg-teal-600 shadow-teal-200">
            <GitBranch size={24} />
          </div>
          <div>
            ...
            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">
              Trang thực hành riêng cho thuật toán Nhánh Cận
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          <select
            ...
            className="bg-slate-50 border border-slate-300 rounded-xl px-4 py-2 font-bold text-slate-700 outline-none focus:ring-2 ring-teal-500"
          >
      ...
      {showSolution && rows.length > 0 && (
        <div className="mb-4 rounded-2xl border border-amber-200 bg-amber-50/90 px-4 py-3 text-sm text-amber-950">
          ...
          <p className="text-amber-900/90 leading-relaxed">
            Bảng bên phải là kết quả đúng theo Nhánh Cận và đề đang chọn. Chọn "Luyện tập"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        <div className="lg:col-span-4 space-y-6">
          ...
          <div className="bg-slate-800 p-6 rounded-3xl text-white shadow-xl">
            <h4 className="text-xs font-black text-slate-400 uppercase mb-3 flex items-center gap-2">
              <BookOpen size={14} className="text-teal-300" /> Nhắc bài
            </h4>
            <p className="text-sm leading-relaxed text-slate-300 italic">
              Nhánh cận: mở rộng theo f(n)=g(n)+h(n), lưu lời giải tạm thời tốt nhất để
              cắt các nhánh có g(v) lớn hơn cận hiện tại.
            </p>
          </div>
        </div>
        ...
        >
            ...
            <table className="w-full text-left">
              <thead className="bg-slate-50 border-b border-slate-200">
                <tr className="text-slate-500 text-[10px] font-black uppercase tracking-widest">
                  ...
                  <th className="p-4 border-r border-slate-100">DS L1</th>
                ...
                {(showSolution ? rows : history).map((row, rowIdx) => {
                  if (row.isGoal) {
                    return (
                      ...
                      >
                        ...
                        <td className="p-4 text-emerald-800 font-bold" colSpan={7}>
                          {row.goalText}
                          {row.listL ? ` | L: ${row.listL}` : ""}
                        </td>
                      </tr>
                    );
                  }
                  ...
                  if ((row.details || []).length === 0) {
                    return (
                      ...
                      >
                        <td className="p-4 border-r border-slate-100 align-top font-black text-slate-900">
                          {row.expand}
                        </td>
                        <td className="p-4 border-r border-slate-100 text-slate-500 italic">
                          {row.deadEndText || "Khong co trang thai con hop le"}
                        </td>
                        <td className="p-4 border-r border-slate-100">-</td>
                        <td className="p-4 border-r border-slate-100">-</td>
                        <td className="p-4 border-r border-slate-100">-</td>
                        <td className="p-4 border-r border-slate-100">-</td>
                        <td className="p-4 border-r border-slate-100 font-mono text-[12px] font-black text-teal-700">
                          {row.listL1 || "-"}
                        </td>
                        <td className="p-4 font-mono text-[12px] font-black text-teal-700">
                          {row.listL || "-"}
                        </td>
                      </tr>
                    );
                  ...
                  return row.details.map((d, dIdx) => (
                    ...
                    >
                      ...
                      {dIdx === 0 && (
                        <td
                          rowSpan={row.details.length}
                          className="p-4 border-r border-slate-100 align-top font-mono text-[12px] font-black text-teal-700"
                        >
                          {row.listL1 || "-"}
                        </td>
                      )}
                      {dIdx === 0 && (
                        <td
                          rowSpan={row.details.length}
                          className="p-4 align-top font-mono text-[12px] font-black text-teal-700"
                        >
                          {row.listL || "-"}
                        </td>
                      )}
                  ...
                  <>
                    {currentRow.isGoal ? (
                      <tr className="border-b border-teal-100 bg-teal-50/40">
                        <td className="p-2 border-r border-teal-100">
                          <input
                            value={inputs.expand}
                            onChange={(e) => setInputs((p) => ({ ...p, expand: e.target.value }))}
                            disabled={validationErrors.length > 0}
                            className={`w-full p-2 border rounded-xl text-center font-black outline-none focus:ring-2 ring-teal-500 shadow-sm ${
                        ...
                        <td className="p-2" colSpan={6}>
                          <input
                            ...
                            disabled={validationErrors.length > 0}
                            className={`w-full p-2 border rounded-xl outline-none focus:ring-2 ring-teal-500 shadow-sm ${
                              validationErrors.length > 0 ? "opacity-50 cursor-not-allowed" : ""
                            }`}
                            placeholder="TTKT, tim duoc duong di tam thoi, do dai..."
                          />
                        </td>
                      </tr>
                    ) : (
                      <>
                        ...
                        {(inputs.rows || []).length > 0 ? (
                          (inputs.rows || []).map((_, detailIdx) => (
                            <tr key={`input-${detailIdx}`} className="border-b border-teal-100 bg-teal-50/40">
                              {detailIdx === 0 && (
                                <td rowSpan={inputs.rows.length} className="p-2 border-r border-teal-100 align-top text-center text-xs text-slate-500 font-bold">
                                  n dong
                                </td>
                              )}
                              <td className="p-2 border-r border-teal-100">
                              ...
                              {detailIdx === 0 && (
                                <td rowSpan={inputs.rows.length} className="p-2 border-r border-teal-100 align-top">
                                  <input
                                    value={inputs.l1}
                                    onChange={(e) => setInputs((p) => ({ ...p, l1: e.target.value }))}
                                    disabled={validationErrors.length > 0}
                                    className={`w-full p-2 border rounded-xl font-mono outline-none focus:ring-2 ring-teal-500 shadow-sm ${
                                      validationErrors.length > 0 ? "opacity-50 cursor-not-allowed" : ""
                                    }`}
                                    placeholder="DS L1"
                                  />
                          ...
                          <tr className="border-b border-teal-100 bg-teal-50/30">
                            <td className="p-2 text-center text-slate-400 italic text-xs" colSpan={8}>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {!showSolution && !completed && (
                <button
                  ...
                  onClick={handleCheck}
                  disabled={validationErrors.length > 0}
                  className={`px-8 py-3 rounded-2xl text-white font-black shadow-lg transition-transform active:scale-95 bg-teal-600 shadow-teal-200 ${
                    validationErrors.length > 0 ? "opacity-50 cursor-not-allowed" : ""
                  }`}
                >
                  KIỂM TRA (ENTER)
          ...
          {(showSolution || completed) && finalPath.length > 0 && (
            <div className="px-4 pb-4">
              <div className="px-4 py-2 rounded-xl text-xs font-bold bg-teal-100 text-teal-900">
