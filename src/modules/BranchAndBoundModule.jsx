...
export default function BranchAndBoundModule() {
  ...
  const [graphIdx, setGraphIdx] = useState(() => {
    ...
    return branchBoundGraphsData[idx] ? idx : 0;
  });
  const [selectedLevel, setSelectedLevel] = useState(() => {
    const idx = initialSession?.graphIdx ?? 0;
    return branchBoundGraphsData[idx]?.level ?? (branchBoundGraphsData[0]?.level || 1);
  });
      ...
      <div className="sticky top-[-24px] z-20 bg-slate-50/80 backdrop-blur-md pb-4 mb-4">
        <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-200 flex flex-wrap justify-between items-center gap-4">
          <div className="flex items-center gap-3">
            <div className="p-2 rounded-lg bg-teal-600 text-white shadow-md shadow-teal-200">
              <GitBranch size={20} />
            </div>
            <div>
              ...
              <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest mt-1">
                Luyện tập giải bài toán tìm đường đi tối ưu
              </p>
            </div>
          </div>
